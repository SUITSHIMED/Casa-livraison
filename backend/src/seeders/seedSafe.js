...
const seed = async () => {
  ...
  try {
    ...
    // Only create tables if they don't exist (won't delete existing data)
    ...
    const existingUser = await User.findOne({ where: { email: "md@test.com" } });
    if (!existingUser) {
      await User.create({
        name: "md",
        email: "md@test.com",
    ...
    if (restaurantCount === 0) {
      ...
      console.log("✓ Restaurants and menu items already exist");
    }
